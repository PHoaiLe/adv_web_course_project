...
export async function GET_getAllClassesOfTeacher()
{
...
async function sendGetAllClassesRequest()
{
    try
    {
        const url = process.env.TEACHER_GET_ALL_CLASSES
...
export async function GET_getJoinedClassesOfTeacher()
{
    const {isError, response} = await sendGetJoinedClassesRequest();
...
async function sendGetJoinedClassesRequest()
{
    try
    {
        const url = process.env.TEACHER_GET_JOINED_CLASSES
        const response = await fetch(url, {
            method: 'GET',
            credentials: 'include',
            headers: {

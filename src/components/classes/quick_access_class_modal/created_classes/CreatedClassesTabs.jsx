...
function CreatedClassesTabs({CreatedClasses})
{
    ...
    useEffect(() => 
    {
        if(CreatedClasses !== undefined && CreatedClasses.length > 0)
        {
            const initialClassStates = []
            CreatedClasses.forEach((value, index) =>
            {
                const item = 
                {
                    class: value,
            ...
            const initialOpenMoreButtons = new Array(CreatedClasses.length).fill(false)
    ...
    const displayClasses = CreatedClasses.map((value, index) =>
    {
        return (
